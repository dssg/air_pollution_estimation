...
def load_video_names_from_s3(ref_file, paths, s3_credentials: dict):
    ...
    dl = DataLoaderS3(s3_credentials=s3_credentials, bucket_name=paths["bucket_name"])
    ...
    if dl.file_exists(annotations_path):
        annotation_video_names = dl.read_json(
