...
def perform_detections_in_single_image(image_capture, params, paths, s3_credentials, selected_labels=None):
    sess = tf.Session()
    model_initializer, init_data, detection_model = initialize_tensorflow_model(params, paths, s3_credentials, sess)
    boxes, labels, confs = detect_objects_in_image(image_capture, paths, detection_model, model_initializer, init_data, sess, selected_labels)
    sess.close()
