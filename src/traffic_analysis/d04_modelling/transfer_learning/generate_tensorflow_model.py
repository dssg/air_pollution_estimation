...
class YoloV3(object):
    ...
    def predict(self, feature_maps):
        ...
        then produce the output predictions at the test stage.
        '''
...
class VehicleDetector(object):
    """ creates a model using yolov3 as its base with a transfer_learning learning layer on top
    """
    ...
    def forward_thru_yolov3(self, inputs, is_training=False, reuse=False):
        # the input img_size, form: [height, weight]
        # it will be used later
        self.img_size = tf.shape(inputs)[1:3]
        # set batch norm params
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('yolov3_head'):
                    inter1, net = yolo_block(route_3, 512)
                    yolo_feature_1 = slim.conv2d(net, 3 * (5 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    yolo_feature_1 = tf.identity(yolo_feature_1, name='yolo_feature_1')
                    ...
                    inter2, net = yolo_block(concat1, 256)
                    yolo_feature_2 = slim.conv2d(net, 3 * (5 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    yolo_feature_2 = tf.identity(yolo_feature_2, name='yolo_feature_2')
                    ...
                    _, yolo_feature_3 = yolo_block(concat2, 128)
                    yolo_feature_3 = slim.conv2d(yolo_feature_3, 3 * (5 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    yolo_feature_3 = tf.identity(yolo_feature_3, name='feature_map_3')

            return yolo_feature_1, yolo_feature_2, yolo_feature_3

    def forward_thru_transfer_learning_layer(self, yolo_features, is_training=False):
        ''' forward pass through the transfer_learning learning layer
        '''
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('transfer_learning_layer'):

                    feature_maps = []
                    count = 1
                    for yolo_feature in yolo_features:
                        inter, net = transfer_learning_layer(yolo_feature)

                        feature_map = slim.conv2d(net, 3 * (5 + self.class_num), 1, stride=1, normalizer_fn=None,
                                                  activation_fn=None, biases_initializer=tf.zeros_initializer())
                        locals()['feature_map_{0}'.format(count)] = tf.identity(feature_map,
                                                                                name='feature_map_%s' % count)
                        count += 1
...
def transfer_learning_layer(inputs):
    filters = 256
    net = conv2d(inputs, filters * 1, 1)
    return net
