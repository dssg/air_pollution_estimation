...
def __get_kedro_context__():
    ...
    from traffic_estimation.run import __kedro_context__
...
def run(tag, env, parallel, runner):
    """Run the pipeline."""
    from traffic_estimation.run import main
...
def build_docs():
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=air_pollution_estimation"]
    )
